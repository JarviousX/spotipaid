...
/** Common BIP39 English words used to detect pasted seed phrases */
const BIP39_COMMON = new Set(
  [
    ...
    "much", "muffin", "mule", "multiply", "muscle", "museum", "music", "must",
    "mutual", "myself", "mystery", "myth", "naive", "name", "napkin", "narrate",
    ...
    "swing", "switch", "sword", "symbol", "symptom", "syringe", "system", "table",
    "tackle", "tag", "tail", "talent", "talk", "tank", "tape", "target",
    "task", "taste", "tattoo", "taxi", "teach", "team", "tell", "ten",
    "tenant", "tennis", "tent", "term", "test", "text", "thank", "that",
    "theme", "then", "theory", "there", "they", "thing", "this", "thought",
    "three", "thrive", "throw", "thumb", "thunder", "ticket", "tide", "tidy",
    "tie", "tiger", "tilt", "timber", "time", "tiny", "tip", "tired",
...
const solanaAddressSchema = z
  ...
  .regex(BASE58_RE, "Solana address must be valid base58");

/**
 * Detect pasted BIP39 seed / recovery phrases (12/15/18/21/24 words).
 */
export function looksLikeSeedPhrase(input: string): boolean {
  const words = input
    .trim()
    .toLowerCase()
    .split(/[\s,]+/)
  ...
  // Require majority of words to look like BIP39 to avoid false positives
  return matches >= Math.ceil(words.length * 0.75);
}

/**
 * Validate a Solana base58 public key address.
 * Rejects seed phrases and invalid base58 / length.
 */
export function validateSolanaAddress(input: string): string {
  const trimmed = input?.trim();
  if (!trimmed) {
    throw new WalletError("Wallet address is required");
  }

  if (looksLikeSeedPhrase(trimmed)) {
    throw new WalletError(
      "Seed phrases are not accepted. Enter a public wallet address only.",
    );
  }

  // Spaces / multiple tokens that aren't a full seed phrase still rejected
  if (/\s/.test(trimmed)) {
    throw new WalletError("Wallet address cannot contain whitespace");
  }

  const result = solanaAddressSchema.safeParse(trimmed);
  if (!result.success) {
    const msg = result.error.issues[0]?.message ?? "Invalid Solana address";
